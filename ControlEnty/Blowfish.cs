...
namespace WpfApp1.ControlEnty
{
    public class Blowfish
    {
        ...
        private byte[] ConvertToByte(string ImageStringData)
        {
            byte[] buffer = new byte[ImageStringData.Length / 2];
            for (int i = 0; i < ImageStringData.Length; i += 2)
            {
                string strimage =""+ ImageStringData[i] + ImageStringData[i + 1];
                buffer[i / 2] = Convert.ToByte(strimage,16);
            }
            return buffer;
        }

        public byte[] Decipher(byte[] idata)
        {
            int num = (idata.Length / 8) * 8;
            byte[] buffer = idata.Clone() as byte[];
            uint xl = 0;
            uint xr = 0;
            for (int i = 0; i < num; i += 8)
            {
                xl = (uint)((((buffer[i] << 0x18) | (buffer[i + 1] << 0x10)) | (buffer[i + 2] << 8)) | buffer[i + 3]);
                xr = (uint)((((buffer[i + 4] << 0x18) | (buffer[i + 5] << 0x10)) | (buffer[i + 6] << 8)) | buffer[i + 7]);
                this.Decipher(ref xl, ref xr);
                buffer[i] = (byte)((xl >> 0x18) & 0xff);
                buffer[i + 1] = (byte)((xl >> 0x10) & 0xff);
                buffer[i + 2] = (byte)((xl >> 8) & 0xff);
                buffer[i + 3] = (byte)(xl & 0xff);
                buffer[i + 4] = (byte)((xr >> 0x18) & 0xff);
                buffer[i + 5] = (byte)((xr >> 0x10) & 0xff);
                buffer[i + 6] = (byte)((xr >> 8) & 0xff);
                buffer[i + 7] = (byte)(xr & 0xff);
            }
            return buffer;
        }
        ...
        private void Decipher(ref uint xl, ref uint xr)
        {
            ...
            uint x = xl;
            uint num2 = xr;
            for (short i = 0x11; i > 1; i = (short)(i - 1))
            {
                x ^= this.P[i];
                num2 = this.F(x) ^ num2;
                num3 = x;
        ...
        public byte[] Encipher(byte[] idata)
        {
            int num = (idata.Length / 8) * 8;
            byte[] buffer = idata.Clone() as byte[];
            uint xl = 0;
        ...
        private uint F(uint x)
        {
            ...
            ulong num5 = 0L;
            num5 += this.S[0, num] + this.S[1, num2];
            num5 ^= this.S[2, num3];
            num5 += this.S[3, num4];
            num5 &= 0xffffffffL;
            return (uint)num5;
        ...
        public static byte[] FixBytes(byte[] bs)
        {
            ...
            int num2 = ((length / 8) + 1) * 8;
            byte[] dst = new byte[num2];
            Buffer.BlockCopy(bs, 0, dst, 0, length);
        ...
        public static string GetString(byte[] bs)
        {
            UnicodeEncoding encoding = new UnicodeEncoding();
            int length = bs.Length;
            char[] chars = new char[encoding.GetCharCount(bs, 0, length - 8)];
            encoding.GetDecoder().GetChars(bs, 0, length - 8, chars, 0);
            string str = new string(chars);
            chars = new char[encoding.GetCharCount(bs, length - 8, 8)];
            encoding.GetDecoder().GetChars(bs, length - 8, 8, chars, 0);
        ...
        public string ReadFromFile(string FilePath)
        {
            ...
            string imageStringData = reader.ReadToEnd();
            reader.Close();
            byte[] idata = this.ConvertToByte(imageStringData);
            return GetString(this.Decipher(idata));
        }

        public void WriteToFile(string FilePath, string InputString)
        {
            byte[] idata = FixBytes(GetBytes(InputString));
            byte[] buffer3 = this.Encipher(idata);
            string str = "";
            StreamWriter writer = new StreamWriter(FilePath, false, Encoding.UTF8);
            for (int i = 0; i < buffer3.Length; i++)
            {
                str = str + buffer3[i].ToString("X2");
            }
            writer.Write(str);
            writer.Close();
        }
